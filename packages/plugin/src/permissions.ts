...
/**
 * Phase 14D source of truth for OpenCode tool -> permission resource mapping.
 *
 ...
 * spawned subagent's DANGEROUS leaf tools are still individually gated by this
 * same plugin, so mapping the spawn tool to "read" cannot bypass policy).
...
export const TOOL_TARGET_FIELDS: Record<string, string[]> = {
  read: ["filePath"],
  edit: ["filePath"],
  write: ["filePath"],
 ...
 * argument field, capped at 2000 chars. Returns undefined when the tool has no
 * field or all fields are empty — the default-deny posture then applies.
 */
